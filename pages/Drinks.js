...
import { getRecipeSearchByCategoryId, getallDrinks } from "@/context/recipe/drinkSlice";
...
const Drinks = () => {
  ...
  const handlePageDetail = async (drinks_id) => {
		console.log("drinks_id", drinks_id);
		// dispatch(getRecipeByRecipeID(recipe_id));
		window.location.href = `/Drink_Recipe/${drinks_id}`
  ...
  const getUserdrinks = useSelector((state) => state?.drinkSlice?.drinks?.data)

	useEffect(() => {
		if(user?.id){
			dispatch(getallDrinks(user?.id))
		}
  }, [user])
  
  useEffect(() => {
		setDrinks(getUserdrinks)
  }, [getUserdrinks])
        ...
        <div className="mobiile-view">
          ...
          <div className="mobiile-view-food bg-black">
            ...
            <div className="search-mobiel">
              ...
              <input type="text" name="" placeholder="Search Drinks" />
            </div>
          </div>
                ...
                <div className="page_fliter_search">
                  ...
                  <select onChange={(e) => handleChange(e.target.value)}>
                      {categories?.map((cate, index) => (
                        <option key={index} selected={cate.id == 2} value={cate.id}>
                          {cate.name}
                        </option>
                      ))}{" "}
                    </select>
