...
import { getallLabcare } from "@/context/recipe/labCareSlice";
...
const Speciality = () => {
  ...
  const handlePageDetail = async (labCare_id) => {
		console.log("labCare_id", labCare_id);
		// window.location.href = `/labCare/${labCare_id}`
  };

  const [data, setData] = useState(null)
  const [user, setuser] = useState(null)

  const [labCare, setLabCare] = useState(null)
  ...
  const getUserLabCare = useSelector((state) => state?.labCare?.labCare?.data)

	useEffect(() => {
		if(user?.id){
			dispatch(getallLabcare(user?.id))
		}
  }, [user])
  
  useEffect(() => {
		setLabCare(getUserLabCare)
  }, [getUserLabCare])
                  ...
                  <div className="page_fliter_select">
                    <select onChange={(e) => handleChange(e.target.value)}>
                      {categories?.map((cate, index) => (
                        <option key={index} selected={cate.id == 4} value={cate.id}>
                          {cate.name}
                        </option>
                      ))}{" "}
                    </select>
                              ...
                              <div className="full-page-drink">
                                ...
                                <div className="sauceimg">
                                  {item.media ? (
                                    ...
                                    <img src="images/3DZTercmEF4.png" />
                                  )}
                                </div>
                              </div>
                            </div>
