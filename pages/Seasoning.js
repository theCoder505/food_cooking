...
const Seasoning = () => {
  ...
  // const handlePageDetail = async (seasoning_id) => {
  //   console.log("seasoning_id", seasoning_id);
  //   // const payload = { seasoning_id: seasoning_id };
  //   try {
  ...
  //         console.log(res, "----res");
  //         router.push({
  //           pathname: `/seasoningRecipe/${seasoning_id}`,
  ...
  const handlePageDetail = async (seasoning_id) => {
    ...
    window.location.href = `/seasoningRecipe/${seasoning_id}`
  }
  ...
  const getUserSeasoning = useSelector((state) => state?.seasoningSlice?.seasoning?.data)

	useEffect(() => {
		if(user?.id){
			dispatch(getallSeasoning(user?.id))
		}
  }, [user])
  
  useEffect(() => {
		setSeasoning(getUserSeasoning)
  }, [getUserSeasoning])
  
  console.log("seasoning================--------------", seasoning)
                ...
                <div className="page_fliter_search">
                  ...
                  <select onChange={(e) => handleChange(e.target.value)}>
                      {categories?.map((cate, index) => (
                        <option key={index} selected={cate.id == 5} value={cate.id}>
                          {cate.name}
                        </option>
                      ))}{" "}
                    </select>
